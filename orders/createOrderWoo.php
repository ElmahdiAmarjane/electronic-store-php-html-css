...
if (isset($data['name']) && isset($data['phone']) && isset($data['address'])) {
    ...
    $clientAddress = htmlspecialchars($data['address']);
    $clientNbrUnits = htmlspecialchars($data['nbrUnits']);
    $OrderProductId = htmlspecialchars($data['productId']);
    ...
    try {
        $order = $woocommerce->post('orders', $orderData);
        print_r($order); // Debugging line to see the response
